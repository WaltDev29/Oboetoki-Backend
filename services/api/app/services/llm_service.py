...
[요구사항]
1. `source_language`: 각 단어의 원본 언어를 판별하여 ISO 언어 코드(예: 'ja' (일본어), 'en' (영어), 'ko' 등)로 작성하세요.
2. `reading`: 원어가 일본어 한자처럼 읽는 법(요미가나, 발음 기호 등)이 따로 표기되어 있거나 필요한 경우, 이 필드에 작성하세요. 영어의 경우 발음 기호를 입력하고, 발음 표기가 불필요한 언어라면 null로 설정하세요.
